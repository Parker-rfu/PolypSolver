...
from model.PolypSolver import PolypSolver
...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        res1,res2= model(image) # forward
        
        
        res = F.upsample(res1+res2, size=gt.shape, mode='bilinear', align_corners=False) # additive aggregation and upsampling
...
def train(train_loader, model, optimizer, epoch, test_path, model_name = 'PVT-CASCADE'):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            loss = alpha * loss_P1 + beta * loss_P2# current setting is for additive aggregation.
            # ---- backward ----
            loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_save', type=str,
                        default='./PolypSolver NO add T2_pth/'+model_name+'/')
